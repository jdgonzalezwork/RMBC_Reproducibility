...
## and the table 4 of  the manuscript ###### 
#########################################################
...
medRMBC=performance_measures(Y,indicesDGP,thetaNew.alpha,
                             ...
                             nkl=1000000,estimated_clusters = estimated_clustersRMBC)
...
MCR0s=medMCLUST$mcr
KLs=medMCLUST$kl[1]
MCRstar=medMCLUST$mcrOutputsClusters
todo=c(MCRs, MCR0s,MCRstar,Specificitys,Sensitivitys,KLs);
perf$MCLUST<-todo
...
write.csv(perf,"Table4.csv",row.names= T)
xtable(perf)
