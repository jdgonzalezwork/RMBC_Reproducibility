...
## add something manual to fixed a 
## bug that appeared for small samples 
# val=list.files("utils/Rfortests/")
# myfun=function(t){source(t)}
# lapply(paste0("utils/Rfortests/",val), FUN=myfun )
...
# This is added because mclust hangs up 
#seedGrid[236]=1001
...
#  nom=paste("simulationResults_",format(Sys.time(), "%B_%d_%Y"), nrep,"_partial.RDATA",sep="")
#  save.image(nom)
}
t2=date()
nom=paste("simulationResults",format(Sys.time(), "%B_%d_%Y"),"_n_rep_", nrep,"_small_N.RDATA",sep="")
save.image(nom)
...
# test.file
# load("RData/simulationResults_16Dec_2021n_rep_1000_small_N.RDATA")
big=F
contaminated=T
